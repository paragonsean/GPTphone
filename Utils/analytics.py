...
def calculate_total_cost_of_llm_from_transcript(messages, cost_per_input_token, cost_per_output_token,
                                                model="gpt-3.5-turbo", check_for_completion=False,
                                                ended_by_assistant=False, completion_input_token_cost=0.000001,
...
def update_execution_details(current_high_level_assistant_analytics_data, run_details):
    total_duration_till_now = current_high_level_assistant_analytics_data["execution_details"][
                                  "average_duration_of_conversation"] * \
...
def update_historical_values(arr, current_run_val, last_updated_at, should_increment, multiplier=0,
...
def update_historical_spread(current_high_level_assistant_analytics_data, run_details):
    current_high_level_assistant_analytics_data["historical_spread"][
        "number_of_conversations_in_past_5_days"] = update_historical_values(
        current_high_level_assistant_analytics_data["historical_spread"]["number_of_conversations_in_past_5_days"], 1,
        current_high_level_assistant_analytics_data["last_updated_at"], should_increment=True)
    current_high_level_assistant_analytics_data["historical_spread"]["cost_past_5_days"] = update_historical_values(
        current_high_level_assistant_analytics_data["historical_spread"]["cost_past_5_days"], run_details['total_cost'],
        current_high_level_assistant_analytics_data["last_updated_at"], should_increment=True)
    logger.info(f"Before updating average duratio {current_high_level_assistant_analytics_data['historical_spread']}")
    current_high_level_assistant_analytics_data["historical_spread"][
        "average_duration_past_5_days"] = update_historical_values(
        current_high_level_assistant_analytics_data["historical_spread"]["average_duration_past_5_days"],
        run_details['conversation_time'], current_high_level_assistant_analytics_data["last_updated_at"],
        should_increment=False, multiplier=
        current_high_level_assistant_analytics_data["historical_spread"]["number_of_conversations_in_past_5_days"][-1])
...
def update_cost_details(current_high_level_assistant_analytics_data, run_details):
    if run_details['cost_breakdown']['transcriber'] > 0:
        current_high_level_assistant_analytics_data["cost_details"][
            "average_transcriber_cost_per_conversation"] = round(((current_high_level_assistant_analytics_data[
                                                                       "cost_details"][
                                                                  ...
                                                                  current_high_level_assistant_analytics_data[
                                                                      'execution_details']["total_conversations"]), 5)
    if run_details['cost_breakdown']['synthesizer'] > 0:
        current_high_level_assistant_analytics_data["cost_details"][
            "average_synthesizer_cost_per_conversation"] = round(((current_high_level_assistant_analytics_data[
                                                                       "cost_details"][
    ...
    current_high_level_assistant_analytics_data["cost_details"]["average_llm_cost_per_conversation"] = round(((
                                                                                                                          current_high_level_assistant_analytics_data[
                                                                                                                              "cost_details"][
                                                                                                                              "average_llm_cost_per_conversation"] * (
                                                                                                                                      current_high_level_assistant_analytics_data[
                                                                                                                                          'execution_details'][
                                                                                                                                          "total_conversations"] - 1) +
                                                                                                                          run_details[
